...
4. `styles/rest.css`, `styles/index.css` 생성
...
1. `ver` : 오래된 선언방식, 이름 중복 가능
2. `let` : 이름 중복 불가능 , 재사용 가능
...
* `객체.속성;`
    ...
    * `input.value` //value는 input전용속성으로 다른 태그에 사용불가(input은 객체이기에 사용가능)
* `객체.속성 = '값';`
    * 객체의 속성 값을 대입한다
    ...
    * `<span>0</<span>` //span의 내용 0을 -> 1로 변경
        * `span.textContent = 1;` // span 원래 내용은 제거하고 1 새로 대입한다.
        * `span.textContent = span.textContent+ 1;` // 기존내용에 +1 대입한다.
    * `<input type="text" value="1">` // input의 내용 1 -> 2로 변경
        * `input.value = 2;` // input 원래 내용1을 제거하고 새로 2의 값을 대입한다.
        * `input.value = input.value + 2` // 기존 input값에 2값을 대입한다.
* `객체.함수();`
    * 객체를 함수방법으로 실행한다
    * ` 객체.함수() = '값';` (X) 잘못된값 예시
* `이벤트="함수();"`
...
1. 제어하고 싶은 HTML 대상 수와 클래스, id 등 확인
...
4. 안전한 작업을 위해 콘솔로 변수 체크하기
    ...
    * `null`이 뜬다면 HTML script 작성위치로 가서
    defer속성추가
...
* from은 웹페이지에서 사용자의 입력 데이터를 서버로 전송하기 위해 범위를 정하며 action은 입력받은 데이터를 처리할 서버 주소 지정하며 method  데이터를 서버로 전송하는 HTTP 방식 지정 속성이다(action과 method는 from에 대한 따라오는 기본값이다.)

`form : 검색, 로그인, 회원가입 등의 특정 주제로 묶여있는 그룹`
`action : 그룹에 작성 또는 선택된 사용자 정보를 전달하는 서버 주소`
`method : 데이터를 보내는 전송 방식(get, post(보안에 대한 설정))`

## input태그와 type
* input은 해당 태그에 다양한 속성에 대한 역을 부여하며 type은 그 태그에 대한 종류값이다.

`input : 사용자가 입력, 선택할때 작성하는 시작 태그 (type 필수)`
`type : **작성 종류에 따라** type 선택`

## name속성
* 지정할 태그에 요소에 대한 이름을 정하는 값이다.

`name : action으로 저장된 사이트서버 위치에 사용자가 입력한 데이터를 전송할때 구분하는 데이터명칭`
...
`입력방식 : 처음부터 입력된 초기값 (쇼핑몰의 수량1)`
`선택양식 : 위 name처럼 데이터 구분명칭`

## id, class속성
* id와 class는 핵심인 태그에 대하여 이름과 정체성을 지정하는 값이며 이 이름으로 다양한 활용이 가능하다.(태그에 대한 위치,CSS에 대한 적용 등등)

`js, css에서 구분하기 위한 디자인 또는 동적 프로그래밍 용도인 이름`